...
typedef struct {
	int i;
	int j;
	int tamPal[4]; // [0] - coluna S / [1] - linha L / [2] - coluna N / [3] - linha O
	int idxPal[4]; // Index da palavra em determinado ponto
}vertice;
	...
	for (int i = 0; i < nVer; i++){	// Percorrendo todos vertices
		for (int j = 0; j < 4; j++){ // Percorrendo pontos posiveis dentro do vertice
			...
			if (auxIdx >= 0){ // Se existe palavra idexada a aquela posicao
				...
				if (j == 2){ // Caso ela esteja numa coluna no norte
					int auxI = ver[i].i - auxTam;
					int auxJ = ver[i].j;
				...
				if (j == 3){ // Caso ela esteja numa linha no oeste
					int auxI = ver[i].i;
					int auxJ = ver[i].j - auxTam;
	...
	*/
	...
	for (int k = 1; k <= n; k++){
		if (ori == 0){ // Caso a palavra tenha que caber em uma coluna
			...
			if (tab[i-k][j] != pal[n-k] && tab[i-k][j] != '#') return 0;
		}
		else if (ori == 3){			
			if (tab[i][j-k] != pal[n-k] && tab[i][j-k] != '#') return 0;
		}
	}
...
int main(){
	...
	while (scanf("%d %d", &m, &n) && !(m == 0 && n == 0)){
		...
		// 2 linhas e colunas a mais(que formam uma especie de moldura).
		m += 2; n += 2;
		char **tab;
		...
		for (i = 1; i < m-1; i++){
			for (j = 1; j < n-1; j++){
				...
				if (ipt == -1){
					...
					v[nVertices].idxPal[2] = -1;
					v[nVertices].idxPal[3] = -1;
		...
		while (palAtual <= nPal){
			...
			while (verAtual <= nVertices && palAtual < nPal && palAtual >= 0){
				...
				while (lado <= 4){
					...
					if (lado == 4){ // Ja percorri todos os pontos possiveis dentro de um vertice
